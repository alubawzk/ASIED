function [ellipses, L, posi, edge, time, time_combine, time_cluster, time_verifiction] = ellipseDetectionByArcSupportLSs(I, tau_p, tau_h)
...
if(size(I,3)>1)
    I = rgb2gray(I);
    %         gauss = fspecial('gaussian', [5 5], 0.19*sqrt(2)^4);% 0.19*sqrt(2)^4
    %         I = imfilter(I, gauss);
    t0 = clock;
    [edge, normals, lsimg, pairGroup, pairGroup_cover, coeff] = generateEllipseCandidates(I, 2, specified_polarity);%1,sobel; 2,canny
    t1 = clock;
else
    t0 = clock;
    [edge, normals, lsimg, pairGroup, pairGroup_cover, coeff] = generateEllipseCandidates(I, 2, specified_polarity);%1,sobel; 2,canny
...
time_combine = etime(t1,t0);

%% Cluster & Goodness
[y, x]=find(edge);
normals = normals';
t2 = clock;
%     Ellipse_error_tolerance = 0.015;
%     [ candidates_ ] = Ellipse_Cluster_WithoutShift( pairGroup', [x, y], normals', pairGroup_cover, Tmin, sigma_L, sigma_w, Mahala_tolerance  );
%     [ candidates_ ] = Ellipse_Cluster( pairGroup', [x, y], normals', pairGroup_cover, Tmin, sigma_L, sigma_w  );
%     [ candidates_ ] = Ellipse_Cluster_WithShift( pairGroup', [x, y], normals', pairGroup_cover, Tmin, sigma_L, sigma_w  );
%     [ candidates_ ] = Ellipse_Cluster_WithoutShift( pairGroup', [x, y], normals', pairGroup_cover, Tmin, sigma_L, sigma_w  );
Clustered_candidates = Get_HomoSets(pairGroup', tau_h, coeff); % 0.035
%     candidates = Get_FinalCandidates_ParametersMean(pairGroup', Clustered_candidates', sigma_L, sigma_w, Ellipse_error_tolerance, pairGroup_cover)';
candidates = Get_FinalCandidates_DistMean(pairGroup', Clustered_candidates', [x, y],normals,tau_p,Tmin);

t3 = clock;
time_cluster = etime(t3,t2);

%%
%     candidates = candidates';%ellipse candidates matrix Transposition
if(isempty(candidates))%表示没有找到候选圆
...
t4 = clock;
%     for xx = 1:length(x)
%         if x(xx) == 240 && y(xx) == 183
%             fprintf('x: %d\n', xx);
%         end
%     end
%     imshow(I);hold on;
%     scatter(175,146,'r');hold off;
[mylabels,labels, ellipses] = ellipseDetection(candidates ,[x, y], normals, unit_dis_tolerance, normal_tolerance, Tmin, angleCoverage, I, tau_p);%后四个参数 0.5% 20° 0.6 180°
%     disp('-----------------------------------------------------------');
%     disp(['running time:',num2str(etime(clock,t0)),'s']);
time_verifiction = etime(clock,t4);
%     time = etime(clock, t0);
time = time_combine+time_cluster*1+time_verifiction*1;
%     labels
%     size(labels)
%     size(y)
warning('on', 'all');
...
function [mylabels,labels, ellipses] = ellipseDetection(candidates, points, normals, distance_tolerance, normal_tolerance, Tmin, angleCoverage, E, Mahala_tolerance)
...
for i = 1 : size(candidates,1)
    ...
    inliers_M = s_dx(ASI_Dist(points(s_dx,:)',candidates(i,1:2)',candidates(i,3:4)',candidates(i,5),1) <= 1.8*Mahala_tolerance);
    inliers = inliers_M(dRosin_square(candidates(i,:),points(inliers_M,:)) <= 1);
    ...
    p_dot_temp = point_ds(candidates(i,:), normals(inliers,:), ellipse_normals);
...
for angleLoop = 1 : length(angles)
    ...
    [L2, L, C, validCandidates] = subEllipseDetection( candidates, points(idx, :), normals(idx, :), distance_tolerance, normal_tolerance, Tmin, angles(angleLoop), E, angleLoop, Mahala_tolerance);
...
function [mylabels,labels, ellipses, validCandidates] = subEllipseDetection( list, points, normals, distance_tolerance, normal_tolerance, Tmin, angleCoverage,E,angleLoop, ASI_tolerance)
...
for i = 1 : size(list, 1)
    ...
    inliers_M = i_dx(labels(i_dx) == 0 & (ASI_Dist(points(i_dx,:)',list(i,1:2)',list(i,3:4)',list(i,5),1) <= 1.8*ASI_tolerance)' );
    inliers = inliers_M(dRosin_square(list(i,:),points(inliers_M,:)) <= distance_tolerance_square);
    ...
    % p_dot_temp = point_ds(list(i,:), normals(inliers,:), ellipse_normals);
    p_cnt = sum(p_dot_temp>0);%无奈之举，做一次极性统计，当改为C代码时注意拟合圆时内点极性的选取问题
    if(p_cnt > size(inliers,1)*0.5)
        %极性相异,也就是内黑外白
        ellipse_polarity = -1;
    ...
    [new_ellipse,new_info] = ASIEF(points(inliers,1),points(inliers,2));
    %          [new_ellipse1,new_info1,A1] = fitEllipse_Test(points(inliers,1),points(inliers,2));
    %           if( i == 11 && angleCoverage == 165)
    %          new_ellipse1(3:4) = new_ellipse1(3:4)/100;
    %          drawEllipses(new_ellipse1',E);
    %           end
    ...
    if (new_info == 1)%如果是用最小二乘法拟合的而得出的结果
        ...
        if ( (((new_ellipse(1) - ellipseCenter(1))^2 + (new_ellipse(2) - ellipseCenter(2))^2 ) <= 16 * distance_tolerance_square) ...
            ...
            %                 newinliers_M = i_dx(labels(i_dx) == 0 & (MahalaDist(points(i_dx,:)',new_ellipse(1:2)',new_ellipse(3:4)',new_ellipse(5),1) <= Mahala_tolerance)' & ((dot(normals(i_dx,:), ellipse_normals, 2)*(-ellipse_polarity)) >= 0.923879532511287));
            %                 newinliers = newinliers_M(labels(newinliers_M) == 0 & (dRosin_square(new_ellipse,points(newinliers_M,:)) <= distance_tolerance_square & ((dot(normals(i_dx,:), ellipse_normals, 2)*(-ellipse_polarity)) >= 0.923879532511287) ) );
            ...
            if (length(newinliers) >= length(inliers))
                ...
                [new_new_ellipse,new_new_info] = ASIEF(points(inliers,1),points(inliers,2));
                if(new_new_info == 1)
    ...
    new_comp = calcuCompleteness(points(inliers,:),new_ellipse(1:2),tbins);
    T = 0.55; % T = 0.87;
    temp = new_comp/360;
    Tmin = T + 0.12*temp;
...
%     [theta, ~] = cart2pol(x(:, 1) - center(1), x(:, 2) - center(2)); %theta为(-pi,pi)的角度，num x 1, 转换为极坐标
theta = atan2(x(:, 2) - center(2), x(:, 1) - center(1));%theta为(-pi,pi)的角度，num x 1
tmin = -pi; tmax = pi; % 设置角度范围
tt = round((theta - tmin) / (tmax - tmin) * tbins + 0.5);%theta的第i个元素落在第j个bin，则tt第i行标记为j，大小num x 1
tt(tt < 1) = 1; tt(tt > tbins) = tbins;
h = histc(tt, 1 : tbins);
h_greatthanzero_num = sum(h>0); % bins中有多少个被填上了
completeness = h_greatthanzero_num*(360 / tbins); % 360du中有多少的完成度
end
%% ================================================================================================================================
