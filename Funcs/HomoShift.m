function [MostReliableNum, HomoLabel, HomoVector] = HomoShift( candidates, reference, inliers_matrix, Homo_label, ...
                                                   candidates_cover, sigma_L, sigma_w, dis_tolerance )
%HomoShift 此处显示有关此函数的摘要
%   输入:
%   candidates: 候选椭圆
%   reference: 当前最佳椭圆索引
...
%     num_candidates = reference;
    ...
    % 将reference加入同源集合
    if num_Homologous ~= 1
        Homo_vector(num_Homologous,:) = reference;
        num_Homologous = num_Homologous + 1;
    end
