                         ...
                         double prec, double p, struct rect * rec )
{
  ...
  */
  ...
  for(i=0; i<reg_size; i++)//用向量内积求在线段方向和与线段方向垂直方向的投影求l,w
    {
      l =  ( (double) reg[i].x - x) * dx + ( (double) reg[i].y - y) * dy;  //向量(reg[i]-center)在直线段方向上的投影
...
double * mylsd(int * n_out, double * img, int X, int Y, int ** reg_img, int * reg_x, int * reg_y)
{
  ...
  double sigma_scale = 0.95; /* Sigma for Gaussian filter is computed as
	...
	for ( int i = 0; i<line_num; i++)
	{
		if( label[i] == 0)//未被分组过
		{
			...
			while(isEnd == 0)
			{
				...
				//head = tail = NULL;
		        bincnt = 0;
				dir_vec1.x = lines[currentLine*8+4]; // dx  (x1,y1,x2,y2,dx,dy,width,polarity)
				dir_vec1.y = lines[currentLine*8+5]; // dy
				if ( lines[currentLine*8+7] == 1)//极性为正
				{
					//将dir_vec1逆时针旋转45°  //为了确定搜索范围
				...
				for ( int j = 1; j<=4; j++)
					for ( int k = 1; k<=4; k++)//在4x4邻域内搜索,沿着直线方向延长搜索4次
					{
						xx = (int)(lines[currentLine * 8 + 2] * scale_zoom + j*dir_vec1.x + k*dir_vec2.x); // x2  (x1,y1,x2,y2,dx,dy,width,polarity) 从尾端开始
						yy = (int)(lines[currentLine * 8 + 3] * scale_zoom + j*dir_vec1.y + k*dir_vec2.y); // y2
						if(xx < 0 || xx >= imgx || yy < 0 || yy >= imgy)//越界
							continue;
						temp = region[yy*imgx+xx];  //region储存的支持点属于哪条线段的标记
			...
			while(isEnd == 0)
			{
				...
				for ( int j = 1; j<=4; j++)
					...
					{
						xx = (int)(lines[currentLine*8+0]* scale_zoom +j*dir_vec1.x+k*dir_vec2.x);
						yy = (int)(lines[currentLine*8+1]* scale_zoom +j*dir_vec1.y+k*dir_vec2.y);
						if(xx < 0 || xx >= imgx || yy < 0 || yy >= imgy)//越界
							continue;
			...
			(*groups).push_back(group_temp); //添加线段分组，按LS方向顺序保存
			temp = (*groups).size()-1;
...
/** Approximate the distance between a point and an ellipse using Rosin distance.
*/
inline double d_Mahalanobis(double *param, double x, double y)
{
	double ae2 = param[2] * param[2];
	double be2 = param[3] * param[3];
	x = x - param[0];
	...
	double U[4] = { cos(-param[4]), -sin(-param[4]), sin(-param[4]), cos(-param[4]) };
	double T[4] = { pow(param[2],2), 0, 0, pow(param[3],2) };
	double S[4] = { pow(U[0],2)*T[0] - pow(U[2],2)*T[3], U[0] * U[2] * T[0] - U[0] * U[2] * T[3],
					U[0] * U[2] * T[0] - U[0] * U[2] * T[3], pow(U[1],2)*T[0] - pow(U[1],2)*T[3] };
	double frac_S = 1 / (S[0] * S[3] - S[1] * S[2]);
	double inv_S[4] = { frac_S*S[3], -frac_S*S[1], -frac_S*S[2], frac_S*S[0] };
	double M_dist = pow(x, 2)*inv_S[0] + pow(y, 2)*inv_S[3] + x*y*(inv_S[1] + inv_S[2]);
	return M_dist;
	//double xp = x*cos(-param[4]) - y*sin(-param[4]);
	//double yp = x*sin(-param[4]) + y*cos(-param[4]);
	//double fe2;
...
//输出
//ellipara
bool calcEllipseParametersAndValidate( double * lines, int line_num, vector<vector<int>> * groups, int first_group_ind,int second_group_ind, double * fit_matrix1, double * fit_matrix2, image_double angles, double distance_tolerance, unsigned int * group_inliers_num, point5d *ellipara, double * Coefficients)
{
	double S[36]; //拟合矩阵S
...
bool calcCombineEllipseParametersAndValidate(double * lines, int line_num, vector<vector<int>> * groups, int first_group_ind, int second_group_ind, double * fit_matrix1, double * fit_matrix2, image_double angles, double distance_tolerance, unsigned int * group_inliers_num, point5d *ellipara, double * Coefficients)
{
	double S[36]; //拟合矩阵S
	//double Coefficients[6] = { 0,0,0,0,0,0 };// ax^2 + bxy + cy^2 + dx + ey + f = 0 
	double param[5], param2[5];
	...
	if (flag1 == TRUE && first_group_inliers.size() >= 0.8*group_inliers_num[0])//靠近最大统计过的内点,通过验证
	{
		if (first_group_inliers.size() >= group_inliers_num[0])//更新组出现过的最大内点数
			group_inliers_num[0] = first_group_inliers.size();
	}
	else
		flag1 = FALSE;
	//第一个组完成验证
...
bool Judge_gc_query_region(int query_index, vector<int>* g_c, double* lines, vector<vector<int>>* groups, double distance_tolerance)
{
	int ind_start, ind_end;
	point2d pointGcs, pointGce, pointGqs, pointGqe, gcs_ls_dir, gce_ls_dir, gqs_ls_dir, gqe_ls_dir;
	double polarity;
	ind_start = (*groups)[(*g_c)[0]][0];//第i组的最开始一条线段索引, g_c中起始group的第一条线段
	ind_end = (*groups)[(*g_c).back()][(*groups)[(*g_c).back()].size() - 1];//第i组的最后一条线段索引 //(*groups)[i].size()-1，g_c中最后一个group
	pointGcs.x = lines[ind_start * 8];
	pointGcs.y = lines[ind_start * 8 + 1];
	gcs_ls_dir.x = lines[ind_start * 8 + 4];
	gcs_ls_dir.y = lines[ind_start * 8 + 5];
	pointGce.x = lines[ind_end * 8 + 2];
	pointGce.y = lines[ind_end * 8 + 3];
	gce_ls_dir.x = lines[ind_end * 8 + 4];
	gce_ls_dir.y = lines[ind_end * 8 + 5];
	ind_start = (*groups)[query_index][0];//第j组的最开始一条线段索引
	ind_end = (*groups)[query_index][(*groups)[query_index].size() - 1];//第j组的最后一条线段索引
	pointGqs.x = lines[ind_start * 8];
	pointGqs.y = lines[ind_start * 8 + 1];
	gqs_ls_dir.x = lines[ind_start * 8 + 4];
	gqs_ls_dir.y = lines[ind_start * 8 + 5];
	pointGqe.x = lines[ind_end * 8 + 2];
	pointGqe.y = lines[ind_end * 8 + 3];
	gqe_ls_dir.x = lines[ind_end * 8 + 4];
	gqe_ls_dir.y = lines[ind_end * 8 + 5];
	polarity = lines[ind_start * 8 + 7]; //i,j两组的极性
	return regionLimitation(pointGcs, gcs_ls_dir, pointGce, gce_ls_dir, pointGqs, gqs_ls_dir, pointGqe, gqe_ls_dir, polarity, -3 * distance_tolerance);
}

//输入
//lines存着x1,y1,x2,y2,dx,dy,length,polarity
//判断新加入的group是否需要调换顺序
void Insert_query_Index(int query_index, vector<int>* g_c, double* lines, vector<vector<int>>* groups)
{
	int g_num = (*g_c).size();
	double x_s_q = lines[(*groups)[query_index][0] * 8 + 0]; //起始点为第一根线段的尾端点(起始点) (x1,y1)
	double y_s_q = lines[(*groups)[query_index][0] * 8 + 1];
	double x_e_q = lines[(*groups)[query_index].back() * 8 + 2];//终止点为最后一跟线段的头端点(终止点) (x2,y2)
	double y_e_q = lines[(*groups)[query_index].back() * 8 + 3];
	for (size_t i = 0; i < g_num; i++)
	{
		double x_s_i = lines[(*groups)[(*g_c)[i]][0] * 8 + 0];
		double y_s_i = lines[(*groups)[(*g_c)[i]][0] * 8 + 1];
		double x_e_i = lines[(*groups)[(*g_c)[i]].back() * 8 + 2];
		double y_e_i = lines[(*groups)[(*g_c)[i]].back() * 8 + 3];
		if (dist(x_s_i,y_s_i,x_e_q,y_e_q) < dist(x_e_i, y_e_i, x_s_q, y_s_q))  //如果顺序不对，就调换次序，将querry代替当前的顺序，后面的一次后移
		{
			(*g_c).insert((*g_c).begin() + i, query_index); //将querry插入当前的位置的group索引
			return;
		}
		(*g_c).push_back(query_index); //将querry插入到group_c的最后位置
	}
}
...
PairGroupList * getValidInitialEllipseSet( double * lines, int line_num, vector<vector<int>> * groups, double * coverages, image_double angles, double distance_tolerance, int specified_polarity, vector<double> *pairCoverages, vector<vector<double>> *vec_coeff)
{
	//加速计算
	...
	/*输出coefficient*/
	//vector<vector<double>> vec_coeff;
	vector<double> temp_coeff;
	double Coefficients[6] = { 0,0,0,0,0,0 };
	/*输出coefficient*/
	...
	for ( i = 0; i<groupsNum; i++)
	{
		if(coverages[i] >= M_4_9_PI )//当组的覆盖角度>= 4pi/9 = 80°, 我们认为具有很大的显著性，可直接拟合提取
		{
			...
			if (specified_polarity == 0 || (lines[(*groups)[i][0]*8+7] == specified_polarity))
			{
				...
				info = calcEllipseParametersAndValidate(lines,line_num,groups,i,-1,(fitMatrixes+i*36),NULL,angles,distance_tolerance,supportInliersNum,&ellipara, Coefficients);
				if (info == FALSE) 
				{
					continue;
				...
				/*保存Coefficients*/
				for (size_t coef_k = 0; coef_k < 6; coef_k++)
				{
					temp_coeff.push_back(Coefficients[coef_k]); //保存的缓存
				}
				(*vec_coeff).push_back(temp_coeff);
				temp_coeff.clear();
				/*保存Coefficients*/
				...
				pairlength++;
				(*pairCoverages).push_back(coverages[i]);
			}
		}
	}
    //selection of pair group hypothesis
	vector<int> group_c;
	double group_c_coverage;
	vector<int> group_temp;
	int temp_g_c;
	vector<vector<int>> groups_combine;
	int g_c_num = 0;
	double *addmatrixes_temp;
	addmatrixes_temp = (double*)malloc(sizeof(double) * 36);
	memset(addmatrixes_temp, 0, sizeof(double) * 36);
	unsigned int * CombinesupportInliersNum = (unsigned int*)malloc(sizeof(int) * 1);//用于存储每个组曾经最大出现的支持内点数量
	memset(CombinesupportInliersNum, 0, sizeof(unsigned int) * 1);//初始化为0.
	vector<double*> AddMatrixes;
	
	//pairCoverages = (double*)malloc(sizeof(double) * 36);
	for (i = 0; i < groupsNum - 1; i++)
	{
		bool flag = true;
		group_c.clear(); // 每次i循环，清零g_c并推入 ith group
		group_c.push_back(i); 
		group_c_coverage = coverages[i];
		for (j = i + 1; j < groupsNum; j++) //for (j = i + 1; j<groupsNum; j++) || for (j = 0; j<groupsNum; j++)
		{
			//if (j == i)
			//{
			//	continue;
			...
			if (specified_polarity == 0 || (lines[(*groups)[i][0] * 8 + 7] == specified_polarity))
			{
				...
				if (lines[(*groups)[i][0] * 8 + 7] == lines[(*groups)[j][0] * 8 + 7] && ((*groups)[i].size() + (*groups)[j].size()) >= 3)
				{
					...
					polarity = lines[ind_start * 8 + 7]; //i,j两组的极性
					if (regionLimitation(pointG1s, g1s_ls_dir, pointG1e, g1e_ls_dir, pointG2s, g2s_ls_dir, pointG2e, g2e_ls_dir, polarity, -3 * distance_tolerance))//ith和jth都在彼此的线性区域内
					{
						/******* 判断jth和g_c在不在彼此的线性区域内 ***********/
						//printf("xxx\n");
						if (group_c_coverage <= 2*M_4_9_PI && Judge_gc_query_region(j, &group_c, lines, groups, distance_tolerance))
						{
							//printf("yyy\n");
							Insert_query_Index(j, &group_c, lines, groups); //将jth插入到合适的位置
							//group_c.push_back(j);
							group_c_coverage = group_c_coverage + coverages[j];
						}
						/******************************************************/
						//if ( i == 2)
						//	drawPairGroup(img,lines,(*groups),i,j);
						//double Coefficients[6] = { 0,0,0,0,0,0 };
						if (calcEllipseParametersAndValidate(lines, line_num, groups, i, j, (fitMatrixes + i * 36), (fitMatrixes + j * 36), angles, distance_tolerance, supportInliersNum, &ellipara, Coefficients))//二次一般方程线性求解，线段的内点支持比例
						{
							/*保存Coefficients*/
							for (size_t coef_k = 0; coef_k < 6; coef_k++)
							{
							...
							pairlength++;
							(*pairCoverages).push_back(coverages[i]+ coverages[j]);
						}
					}
				}
			}
			if (group_c_coverage > 2*M_4_9_PI && group_c.size() > 2 )
			{
				// 拟合一个椭圆,将group_c清零并推入group_i
				// 将group_c中所有LS都加入到一起
				/*flag = false;*/
				groups_combine.push_back(group_temp); //添加线段分组
				temp_g_c = (groups_combine).size() - 1;
				for (int k = 0; k < 36; k++)
					addmatrixes_temp[k] = *(fitMatrixes + group_c[0] * 36 + k);
				for (int m = 0; m < group_c.size(); m++)
				{
					for (int n = 0; n < (*groups)[group_c[m]].size(); n++)
					{
						(groups_combine)[temp_g_c].push_back( (*groups)[group_c[m]][n] );
					}
					if(m!=0)
						addFitMatrix(addmatrixes_temp, (fitMatrixes + group_c[m] * 36), addmatrixes_temp);
				}
				double Coefficients[6] = { 0,0,0,0,0,0 };
				if (calcCombineEllipseParametersAndValidate(lines, line_num, &groups_combine, temp_g_c, -1, addmatrixes_temp, NULL, angles, distance_tolerance, CombinesupportInliersNum, &ellipara, Coefficients))//二次一般方程线性求解，线段的内点支持比例
				{
					/*保存Coefficients*/
					for (size_t coef_k = 0; coef_k < 6; coef_k++)
					{
					...
					pairlength++;
					(*pairCoverages).push_back(group_c_coverage);
				}
				group_c.clear(); //是否与ith里的重复了
				group_c.push_back(i);
				group_c_coverage = coverages[i];
			}
		}
	...
	free(CombinesupportInliersNum);
	free(addmatrixes_temp);
	return pairGroupList;
}
...
{
	if(nrhs!=3)
      mexErrMsgIdAndTxt( "MATLAB:revord:invalidNumInputs","Three input required.");
    else if(nlhs > 6) 
  ...
  //  printf("The number of output arc-support line segments: %i\n",n);
	...
	printf("Groups' total ls num:%i\n",groups_t);*/
	 ...
	 vector<double> pairCoverages;
	 vector<vector<double>> vec_coeff;
	 pairGroupList = getValidInitialEllipseSet(out,n,&groups,coverages,angles,distance_tolerance,specified_polarity, &pairCoverages, &vec_coeff);
	 //printf("The number of combination: %d \n", pairCoverages.size());
	 //printf("The number of vec_coeff：%d \n", vec_coeff.size());
	 if(pairGroupList != NULL)
	 {
		 double * pairGroup_out;
		 double * pairGroup = (double*)malloc(sizeof(double)*(pairGroupList->length) * 5);   // 
		/*printf("pairGroupL_coverages_length = %d\n", pairGroupL_coverages_length);
		  printf("pairGroupList->length = %d\n", pairGroupList->length);*/
		 for (int i = 0; i < pairGroupList->length; i++)
		 {
			 pairGroup[i * 5] = pairGroupList->pairGroup[i].center.x;
			 pairGroup[i * 5 + 1] = pairGroupList->pairGroup[i].center.y;
			 pairGroup[i * 5 + 2] = pairGroupList->pairGroup[i].axis.x;
			 pairGroup[i * 5 + 3] = pairGroupList->pairGroup[i].axis.y;
			 pairGroup[i * 5 + 4] = pairGroupList->pairGroup[i].phi;
		 }
		 plhs[3] = mxCreateDoubleMatrix(5, pairGroupList->length, mxREAL);
		 pairGroup_out = (double*)mxGetPr(plhs[3]);
		 memcpy(pairGroup_out, pairGroup, sizeof(double) * 5 * pairGroupList->length);
		 free(pairGroup);
		 //输出pairGroupL_coverages
		 double *coverage_;
		 plhs[4] = mxCreateDoubleMatrix(pairCoverages.size(), 1, mxREAL);
		 coverage_ = (double*)mxGetPr(plhs[4]);
		 for (int i = 0; i < pairCoverages.size(); i++)
			 coverage_[i] = pairCoverages[i];

		 //输出coefficients
		 double *t_coeffs;
		 plhs[5] = mxCreateDoubleMatrix(vec_coeff.size(), 6, mxREAL);
	     t_coeffs = (double*)mxGetPr(plhs[5]);
		 for (size_t i = 0; i < vec_coeff.size(); i++)
		 {
			 for (size_t j = 0; j < 6; j++)
			 {
				 t_coeffs[j * vec_coeff.size() + i] = vec_coeff[i][j];
				 //printf("%lf\t", vec_coeff[i][j]);
			 }
			 //printf("\n");
		 }
// 		printf("The number of initial ellipses：%i \n",pairGroupList->length);
	 ...
	 else
	 {
		 ...
		 pairGroup_out = (double*)mxGetPr(plhs[3]);
		 pairGroup_out[0] = pairGroup_out[1] = pairGroup_out[2] = pairGroup_out[3] = pairGroup_out[4] = 0;
		 double *coverage_;
...
// 		 printf("The number of initial ellipses：%i \n",0);
	 ...
	 plhs[0] = mxCreateNumericMatrix(imgy,imgx,mxUINT8_CLASS,mxREAL);
	 edgeimg_out = (uchar*)mxGetData(plhs[0]);
...
// 	 printf("edge pixel number: %i\n",edge_pixels_total_num);
	//申请edge_pixels_total_num x 2 来保存每一个边缘点的梯度向量，以列为优先，符合matlab的习惯
	 plhs[1] = mxCreateDoubleMatrix(2,edge_pixels_total_num,mxREAL);
	 gradient_vec_out = (double*)mxGetPr(plhs[1]);
	  for ( int c = 0; c < imgx; c++ )
		 for ( int r = 0; r < imgy; r++)
		 {
			 addr = r*imgx+c;
	...
	if(nlhs == 6)
	{
		...
		plhs[2] = mxCreateDoubleMatrix(imgy,imgx,mxREAL);
		double * ls_img_out = (double*)mxGetPr(plhs[2]);
