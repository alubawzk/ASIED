...
%         inliers = s_dx(MahalaDist(points(s_dx,:)',candidates(i,1:2)',candidates(i,3:4)',candidates(i,5),1) <= 0.02);
    ...
    for i = 1:size(candidates,1)-1
        ...
        ellipseCenter_i = candidates(i,1:2);
        ellipseAxes_i = candidates(i,3:4);
        theta_i = candidates(i,5);
        num_Homologous = 1;
...
%             fprintf('Ou:%d,%d\n',i,j);
%             flag = Judge_Ellipse(candidates(i,:), inliers_matrix{i});
